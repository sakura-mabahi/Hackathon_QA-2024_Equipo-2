...
# Prueba el filtrado por título del producto (seleccionar un título valido para cada prueba)
    def test_filter_by_title(self):
        response = self.get_response(
            {"title": "Classic Navy Blue Baseball Cap"})  # Se cambia "Generic" por un título valido obtenido en Postman al generar una lista de productos
        products = response.json()
        print("Filter by title response:", products)
        self.assertTrue(any("Classic Navy Blue Baseball Cap" in product.get('title', '') for product in products),
                        "No products with title 'Classic Navy Blue Baseball Cap' found")
